...
- **Seeded keys**: With `--seed` or `EUDI_DEV_SEED`, every key the wallet generates derives from that string. Anyone who knows or guesses the seed holds the keys and the CA. The Docker image seeds a wallet whose state lives in memory with a public built-in value, so such a container's CA is known to everyone. Set your own seed, or an empty value for random keys, where that matters.
